...
from transformer_maskgit.videotextdataset import VideoTextDataset, MRICTDataset
...
class TransformerTrainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Swapped out for custom video dataset
        ...
        # original ds returns data in pattern: image, text
        # MRICTDataset returns data in pattern: mr_image, ct_image
        self.ds = MRICTDataset()
        self.valid_ds = MRICTDataset()
        ...
        #     list_train.append(self.ds.paths[self.ds[i]])
        ...
        if len([*self.results_folder.glob('**/*')]) > 0 and yes_or_no('do you want to clear previous experiment checkpoints and results?'):
            rmtree(str(self.results_folder)) # experiment folder needs to be empty
    ...
    def train_step(self):
        ...
        with self.accelerator.autocast():
            # maskgittransformer is adjusted to take ct video and mri "text"
            loss = self.maskgittransformer(video, texts=text, video_frame_mask=mask) # pass text directly as tensor
        ...
        if self.is_main and not (steps % self.save_results_every):
            with torch.no_grad():
                ...
                for model, filename in vaes_to_evaluate:
                    ...
                    # valid_data, text = next(self.valid_dl_iter) same logic as above, just swap text = mri, valid_data = ct
                    text, valid_data = next(self.valid_dl_iter)
                    #print(text)
                    text=text.to(device)
