...
# TODO: Adopt Lookup Free Quantization: https://arxiv.org/pdf/2310.05737.pdf
...
class DiscriminatorBlock(nn.Module):
    def __init__(
        ...
        self.downsample = nn.Sequential(
            # see synthgen_pytorch.test.einops_test.py
...
class Discriminator(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # we're working with square images
        ...
        for ind, (in_chan, out_chan) in enumerate(layer_dims_in_out):
            ...
            blocks.append(block)
            attn_block = None # compatible with module list
        ...
        self.to_logits = nn.Sequential(
            ...
            leaky_relu(),
            Rearrange('b ... -> b (...)'), # flatten to vectors
            nn.Linear(latent_dim, 1), # calculate single logit per image
            Rearrange('b 1 -> b') # flatten to single dimension
        )
    ...
    def forward(self, x):
        ...
        for block, attn_block in zip(self.blocks, self.attn_blocks):
            ...
            if exists(attn_block):
                x, ps = pack([x], 'b c *') # tensors may be 4 or 5 dimensional, (images/videos) pack them to 3 dim and store packing shapes
                x = rearrange(x, 'b c n -> b n c')
                x = attn_block(x) + x # do residual attention 
                x = rearrange(x, 'b n c -> b c n')
                x, = unpack(x, ps, 'b c *') # now unpack with stored packing shapes
...
class ViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # encoder decoder pair with vector quantization
