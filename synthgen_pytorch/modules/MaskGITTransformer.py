...
class MaskGITTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Add mrivit for eval 
        self.mrivit = mrivit.copy_for_eval()
        self.maskgit = maskgit
        ...
        # text conditioning
        # same dimension for video conditioning for convenience
    ...
    def encode_video(self, texts, output_device = torch.device("cuda")):
        """
        ...
        def encode_mri_batch(video, mrivit=t5):
            # video needs to be a batch else the to_patch_emb functions fail because of shape mismatches
            """Wrapper that encodes batch of mri images to batch of tokens with a given vision transformer"""
            ...
            # derive patches
            with torch.no_grad():
                ff_tokens = mrivit.to_patch_emb_first_frame(first_frame)
                rf_tokens = mrivit.to_patch_emb(rest_frames)
            tokens = torch.cat((ff_tokens, rf_tokens), dim = 1)
        ...
        # now downsample this to the needed dimension
        # TODO: try different downsampling strategies
        downsampled = F.interpolate(encoded, size=(3, 6, 128))
    ...
    def sample_images(
        self,
        *,
        # takes video Tensor for mri conditioning
        texts: Union[List[str], str, Tensor] = None,
        batch_size = 1,
        cond_scale = 3.,
        ...
        single_framed_video = self.sample(
            # texts input can be video tensor
            texts = texts,
            num_frames = 1,
    ...
    def sample(
        ...
        *,
        num_frames,
        # texts may be a video tensor for mri conditioning
        texts: Union[List[str], str, Tensor] = None,
        prime_frames = None,
        batch_size = 1,
        cond_scale = 3.,
        ...
        for step in range(self.steps):
            ...
            logits = self.maskgit.forward_with_cond_scale(
                ...
                context = text_embeds, # MRI embeds have same shape
                text_mask = text_mask, # text mask has same shape
                cond_scale = cond_scale
            )
            ...
            if not is_last_step:
                if exists(self.critic):
                    critic_kwargs = dict(
                        video_patch_shape = patch_shape,
                        context = text_embeds, # MRI embeds have same shape
                        text_mask = text_mask, # Video mask has same shape
                        cond_scale = cond_scale
                    )
        ...
        return video # End of function sample
    ...
    def forward(
        ...
        videos = None,
        *,
        texts: Union[List[str], str, Tensor] = None, # Videos can be passed in as tensors
        video_codebook_ids = None,
        video_frame_mask = None,
        text_embeds = None, # Video embeds have same shape as t5
        cond_drop_prob = None,
        ...
        # TODO: we can pass in video embeds directly to save time on encoding maybe?
        ...
        # MRI mask has same shape as text mask
        text_mask = None
        cond_drop_prob = 0
        ...
        # train maskgit with mri conditioning
