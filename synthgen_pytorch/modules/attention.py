"""
Util functions for Custom Transformer Class

Layer Norm: https://arxiv.org/abs/1607.06450
Gated Gaussian Error Linear Unit: https://arxiv.org/abs/2002.05202
Position Encoding Generator: https://arxiv.org/abs/2102.10882
Attention with Linear Bias (ALiBi): https://arxiv.org/abs/2108.12409
Continuous Position Bias: https://arxiv.org/abs/2111.09883
"""
...
# bias-less layernorm leads to greater stability. Used in T5 & PaLM
class LayerNorm(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.gamma = nn.Parameter(torch.ones(dim)) # weight
        self.register_buffer("beta", torch.zeros(dim)) # bias
    
    def forward(self, x):
        return F.layer_norm(x, x.shape[-1:], self.gamma, self.beta)

# Gated Gaussian Error Linear Unit
class GEGLU(nn.Module):
    def forward(self, x):
        x, gate = x.chunk(2, dim = -1)
        return F.gelu(gate) * x

# FF layer to wrap everything together
...
class PEG(nn.Module):
    ...
    @beartype # guarantee that video tensor is 4 dimensional: b d w h
...
class Attention(nn.Module):
    """
    Einsum attention by @Lucidrains
    """
    def __init__(
        self,
        dim,
...
# Wrap everything in Transformer Module
